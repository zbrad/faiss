...
- **sm_121 / DGX Spark**: The GB10 chip pairs a Grace CPU (aarch64/sbsa) with a Blackwell GPU die. Build with `tuned/build.sh gb10` + `tuned/wheel.sh gb10`; requires `libcuvs-gb10-cu132.so` (GPU-codename naming, see [zbrad/cuvs tuned/docs/WHEEL_NAMING.md](https://github.com/zbrad/cuvs/blob/tuned-builds/tuned/docs/WHEEL_NAMING.md)) from [zbrad/cuvs](https://github.com/zbrad/cuvs).
