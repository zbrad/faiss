...
- **sm_121 / DGX Spark**: The GB10 chip pairs a Grace CPU (aarch64/sbsa) with a Blackwell GPU die. Build with `build_wheel_aarch64.sh`; requires `libcuvs-gb10-cu132.so` (GPU-codename naming, see [zbrad/cuvs gpu-build/docs/WHEEL_NAMING.md](https://github.com/zbrad/cuvs/blob/feat/gpu-build-naming/gpu-build/docs/WHEEL_NAMING.md), not yet merged) from [zbrad/cuvs](https://github.com/zbrad/cuvs).
