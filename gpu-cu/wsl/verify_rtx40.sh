#!/bin/bash
# Verify the installed faiss-rtx40-${FAISS_CUDA_TAG} wheel works (CPU + GPU).
# Usage (from PowerShell):
#   wsl -e bash gpu-cu/wsl/verify_rtx40.sh
# To install first, pass --install:
#   wsl -e bash gpu-cu/wsl/verify_rtx40.sh --install
...
VARIANT="${FAISS_VARIANT:-rtx40-${FAISS_CUDA_TAG}}"
# pip normalises hyphens to underscores in wheel filenames
WHEEL_PREFIX="faiss_${VARIANT//-/_}"
WHEEL=$(ls "$FAISS_ROOT"/build_output_rtx40/${WHEEL_PREFIX}-*.whl 2>/dev/null | head -1)

# Fallback: plain "faiss" wheel (no variant)
if [[ -z "$WHEEL" ]]; then
    WHEEL=$(ls "$FAISS_ROOT"/build_output_rtx40/faiss-*.whl 2>/dev/null | head -1)
fi

if [[ "${1:-}" == "--install" ]]; then
    if [[ -z "$WHEEL" ]]; then
        echo "ERROR: No wheel found in $FAISS_ROOT/build_output_rtx40/"
        exit 1
    fi
