...
#
# Build C++ library (libfaiss) — RTX 40 / Ada Lovelace (x86_64, Intel MKL, AVX2/AVX512)
...
echo "========================================="
echo "Building FAISS C++ Library (RTX 40 / Ada Lovelace)"
echo "========================================="
echo "CUDA_HOME: $CUDA_HOME"
echo "CUDA_ARCHS: $CUDA_ARCHS (SM 89, Ada Lovelace)"
...
cmake -B "$BUILD_DIR" \
    ...
    -DFAISS_OUTPUT_NAME=faiss-rtx40-${FAISS_CUDA_TAG} \
    -DFAISS_C_OUTPUT_NAME=faiss_c-rtx40-${FAISS_CUDA_TAG} \
...
mkdir -p _libfaiss_stage_rtx40/
cmake --install "$BUILD_DIR" --prefix _libfaiss_stage_rtx40/ --config Release
...
cp -f "$BUILD_DIR/faiss/libfaiss_avx512.so" _libfaiss_stage_rtx40/lib/ 2>/dev/null || true
cp -f "$BUILD_DIR/c_api/libfaiss_c_avx512.so" _libfaiss_stage_rtx40/lib/ 2>/dev/null || true

echo ""
echo "========================================="
echo "✓ C++ library build complete (RTX 40, CUDA $FAISS_CUDA_VER)"
echo "========================================="
echo "Libraries built in: $BUILD_DIR/faiss/"
echo "  libfaiss-rtx40-${FAISS_CUDA_TAG}.so    (main C++ library)"
echo "  libfaiss_c-rtx40-${FAISS_CUDA_TAG}.so  (C API wrapper)"
echo "  libfaiss_avx2.so / libfaiss_avx512.so  (SIMD variants)"
echo "Staged in: _libfaiss_stage_rtx40/"
