...
#
# Build Python package (faiss-gpu) — RTX 50 / Blackwell (Intel MKL, AVX2/AVX512)
...
echo "========================================="
echo "Building FAISS Python Package (RTX 50 / Blackwell)"
echo "========================================="
...
if [ ! -d "_libfaiss_stage_rtx50" ]; then
    echo "ERROR: libfaiss not staged. Run build_lib_rtx50.sh first."
    exit 1
fi
...
rm -rf "$BUILD_DIR"
cmake -B "$BUILD_DIR" \
    -Dfaiss_ROOT=_libfaiss_stage_rtx50/ \
    -DCMAKE_LIBRARY_PATH="${FAISS_ROOT}/_libfaiss_stage_rtx50/lib" \
    -DCMAKE_SHARED_LINKER_FLAGS="-L${FAISS_ROOT}/_libfaiss_stage_rtx50/lib" \
...
echo ""
echo "========================================="
echo "✓ Python package build complete (RTX 50)"
echo "========================================="
