...
#
# Build Python package (faiss-gpu) — RTX 40 / Ada Lovelace (Intel MKL, AVX2/AVX512)
...
echo "========================================="
echo "Building FAISS Python Package (RTX 40 / Ada Lovelace)"
echo "========================================="
...
if [ ! -d "_libfaiss_stage_rtx40" ]; then
    echo "ERROR: libfaiss not staged. Run build_lib_rtx40.sh first."
    exit 1
fi
...
rm -rf "$BUILD_DIR"
cmake -B "$BUILD_DIR" \
    -Dfaiss_ROOT=_libfaiss_stage_rtx40/ \
    -DCMAKE_LIBRARY_PATH="${FAISS_ROOT}/_libfaiss_stage_rtx40/lib" \
    -DCMAKE_SHARED_LINKER_FLAGS="-L${FAISS_ROOT}/_libfaiss_stage_rtx40/lib" \
...
echo ""
echo "========================================="
echo "✓ Python package build complete (RTX 40)"
echo "========================================="
